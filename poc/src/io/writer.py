...
def write_results(result: PipelineResult, output_dir: str | Path) -> dict[str, Path]:
    ...
    # report.json
    step_timings_data = (
        [t.model_dump() for t in result.step_timings] if result.step_timings else []
    )
    total_duration = (
        sum(t.duration_seconds for t in result.step_timings)
    ...
    failed_steps = (
        sum(1 for t in result.step_timings if t.status == "failed")
        if result.step_timings
        else 0
    )
    skipped_steps = (
        sum(1 for t in result.step_timings if t.status == "skipped")
        if result.step_timings
        else 0
    )
    total_retries = (
        sum(t.retry_count for t in result.step_timings) if result.step_timings else 0
    )

    report = {
        "input_file": result.input_file,
        "raw_segments": len(result.raw_transcript.segments)
        if result.raw_transcript
        else 0,
        "fixed_segments": len(result.fixed_transcript.segments)
        if result.fixed_transcript
        else 0,
        "correction_diffs": len(result.fixed_transcript.diffs)
        if result.fixed_transcript
        else 0,
        ...
        "failure_rate": round(failed_steps / total_steps, 4)
        if total_steps > 0
        else 0.0,
